...
xe_nk_init(struct xe_platform *plat)
{
    ...
    struct nk_font *karla = nk_font_atlas_add_from_file(&g_nuk.atlas, "./assets/fonts/Karla-Regular.ttf", 14, 0);
...
xe_nk_render(void)
{
    ...
    nk_draw_foreach(cmd, &g_nuk.ctx, &cmd_buf) {
        if (cmd->elem_count == 234092830) {
            xe_gfx_rops_set((xe_gfx_rops){
                    .clip = {
                        .x = (int)cmd->clip_rect.x,
                        .y = g_nuk.plat->viewport_h - (int)(cmd->clip_rect.y + cmd->clip_rect.h),
